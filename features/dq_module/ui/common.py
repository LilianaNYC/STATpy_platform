...
from ..domain.grading import (
    SEV_NOTE,
    MISSING_BUCKETS, MISSING_BUCKET_COLORS, MISSING_BUCKET_TONES,
    MISSING_BUCKET_RANGES, MISSING_BUCKET_NOTE, missing_bucket,
    TEST_META_BASE, verdict_for, detect_count_anomalies,
)
...
# ── Drift stat-test metadata (UI view over domain.grading) ────────────────
# Numeric thresholds (TEST_META_BASE + level fns) and verdict_for live in
# domain/grading.py; here we layer on the per-test number formatters (fmt).
_TEST_FMT = {
    "psi":      lambda v: "—" if v is None else fmt(v, 3),
    "ks_p":     lambda v: "—" if v is None else fmt(v, 4),
    "anova_p":  lambda v: "—" if v is None else fmt(v, 4),
    "cohens_d": lambda v: "—" if v is None else ("+" if v >= 0 else "") + fmt(v, 3),
    "js_div":   lambda v: "—" if v is None else fmt(v, 3),
}
TEST_META = [{**t, "fmt": _TEST_FMT[t["key"]]} for t in TEST_META_BASE]
...
# ── Misc shared logic ─────────────────────────────────────────────────────
# Severity buckets, the drift verdict, and anomaly detection now live in
# domain/grading.py (re-exported via the import at the top of this file).
...
# ── Export menu (top-right of each page, STATpy-platform integration) ──────
#
# The standalone HTML build shipped a top-bar "Export PDF ▾" split button
# (renderer.py) with "current tab / all tabs". The platform shell has no
# top-bar, so each DQ page mounts its own Export dropdown in the top-right
# corner via export_bar(); register_export() wires the four callbacks.
#   - This page  → PDF   : client-side html2pdf on the live .content element
#   - All pages  → HTML  : server-side full report (bundled renderer)
#   - All pages  → Excel : server-side metrics workbook (render_excel)
# IDs are namespaced dqd-<key>-export-* (one independent menu per page).

def export_bar(key: str, label: str, metrics: dict) -> html.Div:
    """A pinned top-right "Export ▾" dropdown for one DQ page.

    ``key`` namespaces the component ids (must match register_export); ``label``
    is the human page name baked into PDF filenames; ``metrics`` supplies the
    snapshot quarter for filenames. Pure layout — no callbacks here.
    """
    q = (metrics or {}).get("latest_quarter") or ""

    def _item(item_id, icon, lab, sub):
        return html.Button(
            [html.Span(icon, className="ic"),
             html.Span([html.Span(lab, className="lb"),
                        html.Span(sub, className="sb")],
                       style={"display": "flex", "flexDirection": "column"})],
            id=f"dqd-{key}-export-{item_id}", n_clicks=0,
            className="dqd-export-item")

    menu = html.Div([
        html.Div("This page", className="dqd-export-sep"),
        _item("pdf", "\U0001f4c4", "Download as PDF", "The page you're viewing"),
        html.Div("Whole dashboard", className="dqd-export-sep"),
        _item("html", "\U0001f5c2", "All pages — HTML report",
              "Self-contained interactive file"),
        _item("xlsx", "⬇", "All pages — Excel", "Metrics workbook (.xlsx)"),
    ], id=f"dqd-{key}-export-menu", className="dqd-export-menu")

    return html.Div([
        html.Div([
            html.Button(["⬇️ Export ",
                         html.Span("▾", style={"fontSize": "10px"})],
                        id=f"dqd-{key}-export-toggle", n_clicks=0,
                        className="dqd-export-toggle"),
            menu,
        ], className="dqd-export-wrap"),
        dcc.Download(id=f"dqd-{key}-export-dl-html"),
        dcc.Download(id=f"dqd-{key}-export-dl-xlsx"),
        html.Div(id=f"dqd-{key}-export-sink-toggle", style={"display": "none"}),
        html.Div(id=f"dqd-{key}-export-sink-pdf", style={"display": "none"}),
    ], className="dqd-export-bar",
       **{"data-page": label, "data-quarter": q})

# register_export() — the export callback wiring + its client-side JS —
# now lives in callbacks/export.py (callbacks own glue); export_bar above
# is pure layout.
