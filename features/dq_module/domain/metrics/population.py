...
def build_crr_facility_sample(cur_df: pd.DataFrame, prior_df, cfg: dict,
                              key_vars: list[str],
                              per_cell: int = 5,
                              max_sample: int = 400) -> dict:
    """Sample of retained facilities for the CRR drilldown table.

    Aligned with the CRR Migration Matrix on two critical points so the cell
    counts always match the filtered-sample counts:

      1. **Same representative CRR.** Each facility's CRR is taken from the row
         with the maximum Balance value for that facility — identical to
         `build_crr_analysis._per_fcl`. Using a different representative
         (e.g. the first row) would assign different transitions to the same
         facility than the matrix does and produce mismatched counts.
      2. **Stratified coverage.** Up to `per_cell` facilities are taken from
         each (prior, current) transition bucket *before* padding with extras.
         This guarantees that every non-empty matrix cell has at least
         `min(per_cell, cell_count)` facilities present in the sample, so the
         drilldown table is never empty for a cell the matrix counts.

    Total rows = sum_over_cells(min(per_cell, cell_count)) + padding up to
    `max_sample`. With ~70 transitions and per_cell=5, expect ~300-400 rows.
    """
    if prior_df is None or prior_df.empty:
        return {"columns": [], "rows": []}
    ...
    bal_col = "Balance"
    required = [fcl_col, crr_col, bal_col]
    if not all(c in cur_df.columns for c in required):
        return {"columns": [], "rows": []}
    if not all(c in prior_df.columns for c in required):
        return {"columns": [], "rows": []}

    # === Representative row per facility (mirror of build_crr_analysis._per_fcl) ===
    def _per_fcl(d: pd.DataFrame) -> pd.DataFrame:
        d = d.dropna(subset=[fcl_col]).copy()
        d[bal_col] = pd.to_numeric(d[bal_col], errors="coerce").fillna(0)
        idx = d.groupby(fcl_col)[bal_col].idxmax()
        return d.loc[idx, [fcl_col, crr_col]].rename(columns={crr_col: "crr"})

    cur_rep = _per_fcl(cur_df)
    prv_rep = _per_fcl(prior_df)
    merged = cur_rep.merge(prv_rep, on=fcl_col, suffixes=("_cur", "_prv"), how="inner")
    merged = merged.dropna(subset=["crr_cur", "crr_prv"])
    if merged.empty:
        return {"columns": [], "rows": []}

    # === Stratified sample — per-cell coverage first, padding second ===
    merged["_transition"] = (
        merged["crr_prv"].astype(str) + "|" + merged["crr_cur"].astype(str)
    )
    sampled_ids: list = []
    seen: set = set()
    # Pass 1 — up to per_cell from each transition bucket
    for _, group in merged.groupby("_transition"):
        for fcl in group[fcl_col].head(per_cell).tolist():
            if fcl not in seen:
                sampled_ids.append(fcl)
                seen.add(fcl)
    # Pass 2 — pad with additional retained facilities until max_sample
    for fcl in merged[fcl_col].tolist():
        if len(sampled_ids) >= max_sample:
            break
        if fcl not in seen:
            sampled_ids.append(fcl)
            seen.add(fcl)
    sampled_ids = sampled_ids[:max_sample]
    if not sampled_ids:
        return {"columns": [], "rows": []}

    # === Pull key-column values for the sampled facilities ===
    sample_cols = [c for c in key_vars if c in cur_df.columns]
    cur_subset = (
        cur_df[cur_df[fcl_col].isin(set(sampled_ids))]
        .drop_duplicates(subset=[fcl_col])
        .copy()
    )
    for c in cur_subset.select_dtypes(include=["datetime64[ns]", "datetimetz"]).columns:
        cur_subset[c] = cur_subset[c].dt.strftime("%Y-%m-%d")
    cur_subset = cur_subset.astype(object).where(pd.notnull(cur_subset), None)

    # Index by FCL for stable ordered lookup
    cur_indexed = cur_subset.set_index(fcl_col)
    transition_map = merged.set_index(fcl_col)["_transition"].to_dict()

    rows: list[dict] = []
    for fcl in sampled_ids:
        if fcl not in cur_indexed.index:
            continue
        record: dict = {}
        for c in sample_cols:
            if c == fcl_col:
                record[c] = fcl
            elif c in cur_indexed.columns:
                record[c] = cur_indexed.at[fcl, c]
            else:
                record[c] = None
        record["_crr_transition"] = transition_map.get(fcl)
        rows.append(record)

    return {"columns": sample_cols, "rows": rows}
