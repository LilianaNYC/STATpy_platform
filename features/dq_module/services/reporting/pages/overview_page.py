...
function renderOverview() {{
  ...
  const cards = [
    ...
    {{tab:'rules',      title:'Business Rules & Gov.', icon:'⚖️',
      lines:[`DQ Rating: ${{gov.dq_rating||'—'}}`,`Rules Failed: ${{rules.rules_failed||0}} (${{rules.critical_failures||0}} crit.)`,`Recommendations: ${{(gov.recommendations||[]).length}}`],
      status: ratingColor}},
