"""Smoke tests for the LGD Performance page."""
...
def _render_lgd_content_with(**overrides):
    ...
    from STATpy_platform.features.monitoring.domain.lgd import set_lgd_metrics

    cycle = data["lgd_observations_by_cycle"]["CCAR 2026"]
    set_lgd_metrics(cycle.get("metrics_store"), cycle.get("quarters"))
    try:
        return page.render_lgd_performance_content(
            data, "LGD Model A", "All", "2026Q3", {},
...
def _render_lgd_content():
    return _render_lgd_content_with()
...
def test_lgd_main_overview_and_conclusion_render():
    layout = _render_lgd_content()
    ...
    assert "lgd-dashboard-overview" in ids
    assert "lgd-conclusions-verdict" in ids
    assert text.index("Dashboard Main Overview") < text.index("RAG Assignment")


def test_lgd_subnav_has_overview_and_conclusion_group_first():
    layout = page.build_layout()
    text = " ".join(_collect_text(node) for node in layout)
...
def test_apply_lgd_filters_clears_the_reviewer_signoff_draft(monkeypatch):
    from dash import no_update

    import STATpy_platform.features.monitoring.callbacks.lgd_performance as cb
    ...
    apply_fn = captured["apply_lgd_filters"]
    applied, notes, compensating, pending, save_status = apply_fn(
        1, "CCAR 2026", "intsevere", "LGD Model A", "All", "2026Q3",
    )
