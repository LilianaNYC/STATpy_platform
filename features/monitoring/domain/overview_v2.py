"""Cross-portfolio data aggregation for the Overview v2 page.
...
RAG_COLUMN_DESCRIPTIONS = {
    ...
    "Overall RAG": "RAG Assignment: the model's headline RAG for the period -- the worst of its component tests.",
...
def _pd_rows(data: dict, reporting_cycle: str) -> list[dict[str, Any]]:
    ...
    for quarter in quarters:
        for model_name in data.get("model_names", []):
            ...
            ctx = PdFilterContext(quarters=quarters, models={model_name}, segment="all", monitoring_point=quarter)
...
def _pd_segment_rows(data: dict, reporting_cycle: str) -> list[dict[str, Any]]:
    ...
    the Segments chapter equivalent of ``_pd_rows``. Only PD has per-segment
    data; LGD/EAD/Loss contribute nothing here."""
    quarters, performance_observations, rating_migration_observations, metrics_store = _pd_cycle_data(data, reporting_cycle)
    set_precomputed_metrics(metrics_store)
...
def _loss_rows(data: dict, reporting_cycle: str) -> list[dict[str, Any]]:
    ...
    model_label = (model_names("loss") or ["Loss Model"])[0]
...
def build_overview_v2_rows(data: dict, reporting_cycle: str) -> list[dict[str, Any]]:
...
# ---------------------------------------------------------------------------
# Segment rows (Chapter 2) -- PD only; LGD/EAD/Loss have no per-segment
# breakdown, so the Segments chapter is PD's book of business sliced by
# segment instead of by model.
# ---------------------------------------------------------------------------


def build_overview_v2_segment_rows(data: dict, reporting_cycle: str) -> list[dict[str, Any]]:
    return _pd_segment_rows(data, reporting_cycle)
...
def resolve_current_segment_rows(rows: list[dict[str, Any]], monitoring_period: str = "All") -> list[dict[str, Any]]:
    return resolve_current_rows(rows, monitoring_period, group_keys=("Segment",))
...
def segment_overview_summary(current_rows: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for row in current_rows:
        segment = row["Segment"]
        rag = effective_rag(row["Overall RAG"])
        if segment not in by_segment or RAG_SCORE.get(rag, 0) >= RAG_SCORE.get(by_segment[segment], 0):
            by_segment[segment] = rag
...
def segment_heatmap_rows(current_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    output = []
    for segment in sorted({row["Segment"] for row in current_rows}):
        segment_rows = [row for row in current_rows if row["Segment"] == segment]
...
def segment_top_findings(current_rows: list[dict[str, Any]], limit: int | None = None) -> list[dict[str, Any]]:
    ...
    findings.sort(key=lambda row: (_finding_rank(row["RAG"]), row["Segment"], row["Metric"]))
    return findings[:limit] if limit is not None else findings
...
def governance_summary(current_rows: list[dict[str, Any]], findings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    metric_counts = Counter(row["Metric"] for row in driver_findings)
    ...
    for row in findings:
        ...
        if key in seen:
            continue
        seen.add(key)
        drivers = [f["Metric"] for f in findings if f["RAG"] == "Red" and (f["Model Group"], f["Model"]) == key]
        escalations.append({
            "Model Group": row["Model Group"],
            "Model": row["Model"],
            "Monitoring Period": row["Monitoring Period"],
            "Drivers": ", ".join(sorted(set(drivers))),
        })
...
def segment_governance_summary(current_rows: list[dict[str, Any]], findings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    findings_segments = {row["Segment"] for row in findings}
    clean_segments = sorted({row["Segment"] for row in current_rows if row["Segment"] not in findings_segments})
    ...
    for row in findings:
        ...
        drivers = [f["Metric"] for f in findings if f["RAG"] == "Red" and f["Segment"] == row["Segment"]]
        escalations.append({
            "Segment": row["Segment"],
            "Monitoring Period": row["Monitoring Period"],
            "Drivers": ", ".join(sorted(set(drivers))),
    ...
    if total == 0:
        ...
        if escalations:
            escalation_count = len(escalations)
            escalation_names = ", ".join(row["Segment"] for row in escalations)
            requires = "requires" if escalation_count == 1 else "require"
            sentences.append(
