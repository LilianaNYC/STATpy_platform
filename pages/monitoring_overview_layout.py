"""General monitoring overview page for dashboard1."""
...
from ..data.transformations import filter_pd_periods_by_range, pd_tone_class
...
def _rag_trend_graphs(rows: list[dict], rag_column: str, range_store: dict | None = None) -> html.Div:
    ...
    return html.Div(
        ...
        children=[
            html.Div(
                className="overview-trend-panel",
                children=[
                    dcc.Graph(
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        dcc.Dropdown(
                            ...
                            clearable=False,
                            className="overview-dropdown overview-compact-dropdown",
                        ),
...
def page_layout(data: dict, overview_rows: list[dict] | None = None) -> list:
    overview_rows = overview_rows or build_overview_rows(data)
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            className="monitoring-controls overview-controls",
                            children=[
                                html.Div(
                                    className="monitoring-filter",
                                    children=[
                                        html.Label("Monitoring Period", htmlFor=PERIOD_ID),
                                        dcc.Dropdown(id=PERIOD_ID, options=_dropdown_options(options["periods"]), value="All", clearable=False, className="overview-dropdown"),
                                    ],
                                ),
                                html.Div(
                                    className="monitoring-filter",
                                    children=[
                                        ...
                                        dcc.Dropdown(id=MODEL_GROUP_ID, options=_dropdown_options(options["groups"]), value="All", clearable=False, className="overview-dropdown"),
                                    ],
                                ),
                                html.Div(
                                    className="monitoring-filter",
                                    children=[
                                        html.Label("Model", htmlFor=MODEL_ID),
                                        dcc.Dropdown(id=MODEL_ID, options=_dropdown_options(options["models"]), value="All", clearable=False, className="overview-dropdown"),
                                    ],
                                ),
                                html.Div(
                                    className="monitoring-filter",
                                    children=[
                                        ...
                                        dcc.Dropdown(id=SEGMENT_ID, options=_dropdown_options(options["segments"]), value="All", clearable=False, className="overview-dropdown"),
                                    ],
                                ),
                                _build_overview_subnav(),
                            ],
                        ),
                    ],
                ),
