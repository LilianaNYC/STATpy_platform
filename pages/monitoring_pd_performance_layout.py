...
def _build_mev_threshold_chip_row(thresholds: dict | None):
    if not thresholds:
        return None
    ...
    return html.Div(
        className="pd-mev-threshold-chip-row",
        children=[
...
def _mev_marker_legend_item(label: str, date_value: str | None, tone: str) -> html.Div:
    return html.Div(
        className=f"pd-mev-marker-legend-item pd-mev-marker-legend-item-{tone}",
        children=[
            html.Span(className=f"pd-mev-marker-legend-line pd-mev-marker-legend-line-{tone}", **{"aria-hidden": "true"}),
            html.Span(
                className="pd-mev-marker-legend-copy",
                children=[
                    html.Span(label, className="pd-mev-marker-legend-label"),
                    html.Span(format_pd_short_date(date_value), className="pd-mev-marker-legend-date"),
                ],
            ),
        ],
    )
...
def _build_mev_marker_legend_row(model_data: dict, mev_data: dict):
    items = []
    development_date = (mev_data.get("dev_range") or {}).get("development_date")
    if development_date:
        items.append(_mev_marker_legend_item("Development", development_date, "development"))
    severe_scenario_date = model_data.get("severe_scenario_date")
    if severe_scenario_date:
        items.append(_mev_marker_legend_item("Severe scenario", severe_scenario_date, "scenario"))
    if not items:
        return None
    return html.Div(items, className="pd-mev-marker-legend-row", **{"aria-label": "Reference date markers"})
...
def _build_mev_rag_summary_card(selected_models: list[str], catalog: dict) -> html.Article:
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Span(f"Scenario window starts: {summary['severe_quarter'] or 'Unavailable'}", className="pd-mev-rag-model-meta"),
                            html.Span(
                                "Development date: "
                                + (" / ".join(format_pd_short_date(d) for d in summary["development_dates"]) if summary["development_dates"] else "—"),
    ...
    return html.Article(
        ...
        children=[
            ...
            html.Div(format_pd_date_summary(severe_scenario_dates), className="pd-test-value"),
            html.Div("Evaluation window: each model severe scenario date onward", className="pd-test-meta"),
        ...
        rows = [
            html.Div(
                ...
                children=[
                    ...
                    html.Span(" / ".join(format_pd_short_date(d) for d in item["dates"]) if item["dates"] else "—"),
                ],
            )
...
def _build_mev_filter_row(
    ...
    available_mev_names: list[str],
    chart_mev_names: list[str],
    ...
    has_mev_selection = len(chart_mev_names) != len(available_mev_names)
    can_reset = has_model_selection or has_mev_selection or has_mev_range_selection
        ...
        children=[
            html.Div(
                ...
                children=[
                    ...
                    html.P("Refine the MEV charts below by PD model or by individual macroeconomic variable."),
                ],
            ),
            html.Div(
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            html.Label("PD Model"),
                            dcc.Dropdown(
                                id=MEV_MODEL_FILTER_ID,
                                options=[{"label": "All", "value": "all"}] + [{"label": m, "value": m} for m in available_model_names],
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Label("MEV"),
                            dcc.Dropdown(
                                id=MEV_NAME_FILTER_ID,
                                options=[{"label": n, "value": n} for n in available_mev_names],
                                value=chart_mev_names,
                                multi=True,
                                disabled=not available_mev_names,
                                placeholder="Select MEVs",
                            ),
                        ],
                    ),
...
def _build_mev_range_section(data: dict, ctx: PdFilterContext, range_store: dict, mev_filter_store: dict) -> html.Section:
    ...
    for model_index, model_name in enumerate(chart_model_names):
        ...
        for mev_index, (mev_name, mev_data) in enumerate(mev_entries):
            ...
            color = get_pd_mev_chart_color(model_index * 8 + mev_index)
            fig = build_pd_mev_range_figure(model_data, mev_name, mev_data, color, range_store.get("mev"))
            chart_cards.append(
                html.Article(
                        ...
                        html.Div(
                            ...
                            children=[
                                html.Div([
                                    ...
                                    html.Div(f"Reference dates and threshold ranges for {mev_name}.", className="pd-mev-chart-meta"),
                                ]),
                            ],
                        ),
                        *([_build_mev_threshold_chip_row(thresholds)] if thresholds else []),
                        *([row] if (row := _build_mev_marker_legend_row(model_data, mev_data)) is not None else []),
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Strong(data.get("mev_source_file") or "mev_dummy_data.json"),
                    ".",
                ],
            ),
            html.Div(
                ...
                children=[
                    _build_mev_rag_summary_card(selected_models, catalog),
                    build_pd_static_info_card(
                        "Models in scope",
            ...
            _build_mev_filter_row(selected_models, chart_model_names, available_mev_names, chart_mev_names, mev_periods, range_store, ctx),
            *body,
        ],
    )
...
def render_pd_performance_content(data: dict, ctx: PdFilterContext, range_store: dict, trend_horizon_store: dict, mev_filter_store: dict) -> list:
    ...
    section_2_6 = _build_mev_range_section(data, ctx, range_store, mev_filter_store)
